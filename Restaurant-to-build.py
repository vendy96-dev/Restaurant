...
def home_login_button_click(): #Login button on the home screen
    ...
    icon = PhotoImage(file=".\Restaurant.png")
...
def btn_login_window_click(entry_login_name, entry_login_password, login_window): #Login function in login window
    ...
    if correct_username and correct_password:
        ...
        main_menu_window.iconbitmap('.\Restaurant.png')
...
def btn_new_order_click(new_name): #New order button on Main Menu
    cursor_menu.execute('SELECT item FROM menu')
    items = cursor_menu.fetchall()
    item_list = []
...
def btn_place_order_click(new_name, order, quantity):
   ...
   if not quantity.isdigit():
       messagebox.showerror('Error', 'Quantity must be a number and must not be empty!')
...
def exit_application():
    ...
    if exit_app:#Exit the application
        ...
        # cursor_orders.execute("DELETE FROM sqlite_sequence WHERE name = 'orders';")
        # conn_orders.commit()
        # cursor_users.execute('DELETE FROM users')
        # cursor_users.execute("DELETE FROM sqlite_sequence WHERE name = 'user';")
        # conn_users.commit()
...
home_window.iconbitmap('.\Restaurant.ico')
